...
- Ubuntu Linux
- Python 3
- `gi` (GObject Introspection) and Notify for desktop notifications
...
As an alternative to udev rules, you can create a systemd user service to run the script on login.
...
[Service]
Type=oneshot
ExecStart=/usr/bin/python3 /usr/local/bin/gopro_sync.py
